...
fn default_log_filter(global_args: &GlobalArgs) -> eyre::Result<EnvFilter> {
    ...
    if !global_args.debug
        && let Ok(filter) = std::env::var("RUST_LOG")
    {
        return EnvFilter::builder().parse(filter).map_err(Into::into);
    }
    EnvFilter::builder()
        .parse(format!("warn,teamy_mtg={own_level}"))
        .map_err(Into::into)
}
...
#[expect(
    clippy::print_stderr,
    reason = "Tracing initialization failures need a diagnostic even when no subscriber is available"
)]
pub fn init_logging(
