...
pub fn main() -> eyre::Result<()> {
        ...
        figue::builder::<Cli>()
            ...
            .help(move |help| {
                help.version(version)
                    ...
                    .include_implementation_github_url("TeamDman/teamy-mtg", env!("GIT_REVISION"))
            })
            .build(),
    )
    .run()
    ...
    let requested_output_format = if cli.global_args.json {
        Some(cli::output::OutputFormat::Json)
    } else {
        cli.global_args.output_format
    };
